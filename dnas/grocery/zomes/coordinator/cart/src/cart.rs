            ...
            if !link.tag.0.is_empty() {
                ...
                match decode::<QuantityTag>(&tag_bytes) {
                    Ok(tag) => {
                        ...
                        if (tag.status.is_none() || tag.status.as_ref().unwrap() == "active") && tag.quantity > 0 {
...
pub(crate) fn get_checked_out_cart_impl(action_hash: ActionHash) -> ExternResult<Option<CheckedOutCart>> {
