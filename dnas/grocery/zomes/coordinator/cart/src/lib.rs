                ...
                match decode::<QuantityTag>(&tag_bytes) {
                    Ok(tag) => {
                        ...
                        if tag.status.is_none() || tag.status.as_ref().unwrap() == "active" {
                            ...
                            continue; // Skip checked out items
                        }
                    },
...
#[hdk_extern]
pub fn checkout_cart(_: ()) -> ExternResult<ActionHash> {
    let agent_pub_key = agent_info()?.agent_initial_pubkey;
    let current_time = sys_time()?.as_micros() as u64;
    
    // Get all active cart products
    let cart_products = get_cart(())?;
    
    // If cart is empty, return error
    if cart_products.is_empty() {
        return Err(wasm_error!(WasmErrorInner::Guest("Cart is empty".to_string())));
    }
    ...
    // Create a checked out cart entry
    ...
    for link in links {
        if let Some(target_hash) = link.target.into_action_hash() {
            ...
            if cart_products.iter().any(|p| p.product_hash == target_hash) {
                ...
                let product = cart_products.iter().find(|p| p.product_hash == target_hash).unwrap();
...
pub fn get_checked_out_carts(_: ()) -> ExternResult<Vec<CheckedOutCartWithHash>> {
    ...
    for link in links {
        if let Some(cart_hash) = link.target.into_action_hash() {
            // Get the cart entry
            match get_checked_out_cart(cart_hash.clone())? {
                Some(cart) => {
    ...
    warn!("Before filter - Checked out carts count: {}, statuses: {:?}", 
        checked_out_carts.len(), 
        checked_out_carts.iter().map(|c| &c.cart.status).collect::<Vec<_>>());

    // Filter out carts with "returned" status
    checked_out_carts = checked_out_carts.into_iter()
        .filter(|cart| cart.cart.status != "returned")
        .collect();

    // After filtering
    warn!("After filter - Checked out carts count: {}, statuses: {:?}",
        checked_out_carts.len(),
        checked_out_carts.iter().map(|c| &c.cart.status).collect::<Vec<_>>());
...
pub fn get_checked_out_cart(action_hash: ActionHash) -> ExternResult<Option<CheckedOutCart>> {
    match get(action_hash.clone(), GetOptions::default())? {
        Some(record) => {
            let entry = record.entry().as_option().ok_or(wasm_error!(
                WasmErrorInner::Guest("Expected entry".to_string())
            ))?;
            
            match entry {
                Entry::App(app_entry) => {
                    // Convert SerializedBytesError to WasmError manually
                    let checked_out_cart: CheckedOutCart = record.entry()
                        .to_app_option()
                        .map_err(|e| wasm_error!(WasmErrorInner::Guest(format!("Failed to deserialize: {}", e))))?
                        .ok_or(wasm_error!(WasmErrorInner::Guest("Expected app entry".to_string())))?;
...
pub fn return_to_shopping(cart_hash: ActionHash) -> ExternResult<()> {
    ...
    // Get the cart
    let cart = match get_checked_out_cart(cart_hash.clone())? {
        Some(cart) => {
    ...
    match delete_entry(cart_hash.clone()) {
        Ok(action_hash) => {
            ...
            match create_entry(EntryTypes::CheckedOutCart(updated_cart.clone())) {
                Ok(new_hash) => {
                    warn!("Created new history entry with status 'returned': {:?}", new_hash);
    ...
    warn!("Adding {} products back to active cart", cart.products.len());
