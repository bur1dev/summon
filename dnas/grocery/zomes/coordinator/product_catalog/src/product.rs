...
pub fn get_paths(input: &CreateProductInput) -> ExternResult<Vec<Path>> {
    debug!("Creating paths for product: '{}' in category: '{}'", 
           input.product.name, input.main_category);
    debug!("Product details: cat='{}', subcat='{:?}', type='{:?}'", 
           input.main_category, 
           input.subcategory, 
           input.product_type);
    ...
    debug!("Checking dual categorization for product: '{}', dual_cat={:?}", 
           input.product.name,
           input.dual_categorization);
    ...
    if let Some(dual) = &input.dual_categorization {
        ...
        debug!("NO dual categorization found for product: '{}' ({} → {:?} → {:?})",
              input.product.name,
              input.main_category,
              input.subcategory,
              input.product_type);
    ...
    debug!("Created {} paths for product '{}'", paths.len(), input.product.name);
    Ok(paths)
}

pub fn create_links_batch(product_hash: &ActionHash, paths: Vec<Path>) -> ExternResult<()> {
    debug!("Creating {} links for product hash {}", paths.len(), product_hash);
    
    let path_hashes: Vec<EntryHash> = paths.iter()
    ...
    for (i, path_hash) in path_hashes.iter().enumerate() {
        ...
        debug!("Creating link #{} from path {} to product {}", i+1, path_hash, product_hash);
        create_link(
            path_hash.clone(),
            product_hash.clone(),
            LinkTypes::ChunkToProduct,
...
pub fn create_product_batch(products: Vec<CreateProductInput>) -> ExternResult<Vec<Record>> {
   ...
   if let Some(first) = products.first() {
       debug!("First product: '{}', cat='{}', subcat='{:?}', type='{:?}', dual={:?}", 
              first.product.name, 
              first.main_category, 
              first.subcategory, 
              first.product_type,
   ...
   let dual_count = products.iter().filter(|p| p.dual_categorization.is_some()).count();
   debug!("{} out of {} products have dual categorization", dual_count, products.len());
   
   let mut records = Vec::new();
   
   // Process in batches to avoid timeouts
   for (batch_index, product_batch) in products.chunks(BATCH_SIZE).enumerate() {
       debug!("Processing batch {} with {} products", batch_index + 1, product_batch.len());
       
       // Log each product's key categorization details
       for (i, input) in product_batch.iter().enumerate() {
           debug!("Batch product #{}: '{}', cat='{}', subcat='{:?}', type='{:?}', has_dual={}", 
                 i+1,
                 input.product.name,
                 input.main_category, 
                 input.subcategory, 
       ...
       let entry_hashes: Vec<_> = product_batch.iter()
       ...
       for (hash, input) in entry_hashes.iter().zip(product_batch.iter()) {
           debug!("Creating paths for product '{}' ({})", input.product.name, hash);
           let paths = get_paths(input)?;
           create_links_batch(hash, paths)?;
       }

       let batch_records = entry_hashes.iter()
       ...
       debug!("Retrieved {} records for batch {}", batch_records.len(), batch_index + 1);
...
pub fn recategorize_product(input: RecategorizeInput) -> ExternResult<()> {
    debug!("RECAT: Starting recategorization for product: {:?}", &input.product_hash);
    ...
    if let Some(subcategory) = &product.subcategory {
        let sub_path = Path::try_from(format!("categories/{}/subcategories/{}/chunk_1", 
            product.category, subcategory))?;
        all_paths.push(sub_path);
        
        if let Some(product_type) = &product.product_type {
            let type_path = Path::try_from(format!("categories/{}/subcategories/{}/types/{}/chunk_1", 
