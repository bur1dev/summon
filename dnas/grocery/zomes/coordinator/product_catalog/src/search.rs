...
pub fn get_products_by_hashes(hashes: Vec<ActionHash>) -> ExternResult<SearchResult> {
    ...
    if !hashes.is_empty() {
        ...
        debug!("First hash as string representation: {}", format!("{:?}", hashes[0]));
    } else {
...
fn get_records_from_hashes(hashes: Vec<ActionHash>) -> ExternResult<Vec<Record>> {
    ...
    debug!("get_records_from_hashes processing {} hashes in batches of {}", hashes.len(), BATCH_SIZE);
    
    for (batch_index, batch) in hashes.chunks(BATCH_SIZE).enumerate() {
        debug!("Processing batch {} with {} hashes", batch_index + 1, batch.len());
