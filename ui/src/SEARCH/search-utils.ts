...
/**
 * Decode products from API response
 */
...
/**
 * Deduplicate products based on hash value
 */
export function deduplicateProducts(products: Product[]): Product[] {
    const uniqueMap = new Map<string, Product>();

    for (const product of products) {
        const hashKey = JSON.stringify(product.hash);
...
export function groupRelatedProducts(
    ...
    const typeProdHashes = new Set(typeProducts.map(p => JSON.stringify(p.hash)));
    const refHashStr = JSON.stringify(referenceProduct.hash);

    // Filter out products already in typeProducts or the reference product
    const otherMatchedProducts = allProducts.filter(p =>
        JSON.stringify(p.hash) !== refHashStr &&
        !typeProdHashes.has(JSON.stringify(p.hash))
    );
...
export function sortByFuseRelevance(products: Product[], searchTerm: string): void {
    ...
    products.sort((a, b) => {
        const aScore = scoredProducts.find(
            p => JSON.stringify(p.item.hash) === JSON.stringify(a.hash)
        )?.score || 1;

        const bScore = scoredProducts.find(
            p => JSON.stringify(p.item.hash) === JSON.stringify(b.hash)
        )?.score || 1;
